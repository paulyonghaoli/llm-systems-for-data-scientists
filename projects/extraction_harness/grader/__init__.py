"""Autograder for the extraction-harness mini-project.
...
def run_reference(seed: int = 1, n_scenarios: int = 6) -> dict:
    """Entry point used by tools/verify.py and the no-network gate."""
    from grader import reference, rubric

    return rubric.grade(reference.process, seed=seed, n_batches=n_scenarios)
