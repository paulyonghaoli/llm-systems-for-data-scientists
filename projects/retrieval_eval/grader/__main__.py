...
def _report(result: dict, label: str) -> None:
    ...
    for row in result["breakdown"]:
        ...
        print(f"  {row['key']}  {row['name']:<24} {bar:>7}   "
              f"{row['earned']:>5.1f} / {row['points']}")
    print(f"  {'':<3} {'TOTAL':<24} {'':>7}   {result['total']:>5.1f} / 100")
    if result["failures"]:
...
def main() -> int:
    ...
    if args.sweep:
        for seed in range(1, args.sweep + 1):
            result = rubric.grade(reference.evaluate, reference.compare,
                                  seed=seed, n_benchmarks=2)
    ...
    if args.reference:
        result = rubric.grade(reference.evaluate, reference.compare,
                              seed=args.seed, n_benchmarks=args.benchmarks)
    ...
    result = rubric.grade(student.evaluate, student.compare,
                          seed=args.seed, n_benchmarks=args.benchmarks)
